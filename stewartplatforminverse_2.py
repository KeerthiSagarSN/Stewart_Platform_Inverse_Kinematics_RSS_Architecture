...
'''
class Simult1(object):
    
    def ffx(self,p,op_axis,op_new,ob_fixed):
        oc1x,oc1y,oc1z = p
        return ((op_new[0] - oc1x)*(op_new[0] - oc1x) + (op_new[1] - oc1y)*(op_new[1] - oc1y) + (op_new[2] - oc1z)*(op_new[2] - oc1z) - 450*450, (oc1x - ob_fixed[0])*(oc1x - ob_fixed[0]) + (oc1y - ob_fixed[1])*(oc1y - ob_fixed[1]) + (oc1z - ob_fixed[2])*(oc1z - ob_fixed[2]) - 190*190, oc1x - ob_fixed[0])
    
    def ffxsolve(self,op_axis,op_new,ob_fixed):
        return fsolve(self.ffx,(ob_fixed[0],ob_fixed[1],ob_fixed[2]),args = (op_axis,op_new,ob_fixed))

        
Simult1 = Simult1()
oc1x,oc1y,oc1z =  Simult1.ffxsolve(op_axis[:,0],op_new[0,:],ob_fixed[0,:])
oc2x,oc2y,oc2z =  Simult1.ffxsolve(op_axis[:,0],op_new[1,:],ob_fixed[1,:])
'''
'''
class Simult2(object):
    
    def ffx(self,p,op_axis,op_new,ob_fixed,V2_norm):
        oc1x,oc1y,oc1z = p
        return ((op_new[0] - oc1x)*(op_new[0] - oc1x) + (op_new[1] - oc1y)*(op_new[1] - oc1y) + (op_new[2] - oc1z)*(op_new[2] - oc1z) - 450*450, (oc1x - ob_fixed[0])*(oc1x - ob_fixed[0]) + (oc1y - ob_fixed[1])*(oc1y - ob_fixed[1]) + (oc1z - ob_fixed[2])*(oc1z - ob_fixed[2]) - 190*190, (oc1x*V2_norm[0] + oc1y*V2_norm[1] + oc1z*V2_norm[2] ) - (ob_fixed[0]*V2_norm[0] + ob_fixed[1]*V2_norm[1] + ob_fixed[2]*V2_norm[2]), (oc1x*V2_norm[0] + oc1y*V2_norm[1] + oc1z*V2_norm[2] ) - (ob_fixed[0]*V2_norm[0] + ob_fixed[1]*V2_norm[1] + ob_fixed[2]*V2_norm[2]))
    
    def ffxsolve(self,op_axis,op_new,ob_fixed,V2_norm):
        return fsolve(self.ffx,(ob_fixed[0],ob_fixed[1],ob_fixed[2]),args = (op_axis,op_new,ob_fixed,V2_norm))
'''
'''
class Simult2(object):
    ...
    def ffx(self,p,op_axis,op_new,ob_fixed,V2_norm):
        oc1x,oc1y,oc1z = p
        return ((op_new[0] - oc1x)*(op_new[0] - oc1x) + (op_new[1] - oc1y)*(op_new[1] - oc1y) + (op_new[2] - oc1z)*(op_new[2] - oc1z) - l_rod*l_rod, (oc1x - ob_fixed[0])*(oc1x - ob_fixed[0]) + (oc1y - ob_fixed[1])*(oc1y - ob_fixed[1]) + (oc1z - ob_fixed[2])*(oc1z - ob_fixed[2]) - l_cr*l_cr, (oc1x - ob_fixed[0])*V2_norm[0] + (oc1y - ob_fixed[1])*V2_norm[1] + (oc1z - ob_fixed[2])*V2_norm[2] - 0, oc1z <= 190)
    
    def ffxsolve(self,op_axis,op_new,ob_fixed,V2_norm):
        return fsolve(self.ffx,(op_new[0],op_new[1],op_new[2]),args = (op_axis,op_new,ob_fixed,V2_norm))
'''       
...
class Simult2(object):
    ...
    def ffx(self,y,op_axis,op_new,ob_fixed,V2_norm):
        oc1x,oc1y,oc1z = y
        t1 = (op_new[0] - oc1x)*(op_new[0] - oc1x) + (op_new[1] - oc1y)*(op_new[1] - oc1y) + (op_new[2] - oc1z)*(op_new[2] - oc1z) - l_rod*l_rod
        t2 = (oc1x - ob_fixed[0])*(oc1x - ob_fixed[0]) + (oc1y - ob_fixed[1])*(oc1y - ob_fixed[1]) + (oc1z - ob_fixed[2])*(oc1z - ob_fixed[2]) - l_cr*l_cr
        t3 =  (oc1x - ob_fixed[0])*V2_norm[0] + (oc1y - ob_fixed[1])*V2_norm[1] + (oc1z - ob_fixed[2])*V2_norm[2] - 0
        return [t1,t2,t3]
    
    def ffxsolve(self,op_axis,op_new,ob_fixed,V2_norm):
        x0 = np.array([op_new[0],op_new[1],op_new[2]])
        sol = scipy.optimize.root(self.ffx, x0,args = (op_axis,op_new,ob_fixed,V2_norm), method='lm')
        return sol
        #return fsolve(self.ffx,(op_new[0],op_new[1],op_new[2]),args = (op_axis,op_new,ob_fixed,V2_norm))

'''
# Using sympy solver - Works best among scipy and other methods
def Simult3(op_axis,op_new,ob_fixed,V2_norm,l_cr,l_rod):
    oc1x = Symbol('oc1x')
    oc1y = Symbol('oc1y')
    oc1z = Symbol('oc1z')
    return nsolve([(op_new[0] - oc1x)*(op_new[0] - oc1x) + (op_new[1] - oc1y)*(op_new[1] - oc1y) + (op_new[2] - oc1z)*(op_new[2] - oc1z) - l_rod*l_rod,(oc1x - ob_fixed[0])*(oc1x - ob_fixed[0]) + (oc1y - ob_fixed[1])*(oc1y - ob_fixed[1]) + (oc1z - ob_fixed[2])*(oc1z - ob_fixed[2]) - l_cr*l_cr,(oc1x - ob_fixed[0])*V2_norm[0] + (oc1y - ob_fixed[1])*V2_norm[1] + (oc1z - ob_fixed[2])*V2_norm[2] - 0],[oc1x,oc1y,oc1z],[op_new[0],op_new[1],op_new[2]])
...
sol1 =  Simult3(ob_axis[:,0],op_new[0,:],ob_fixed[0,:],V_unit(V1_norm),l_cr,l_rod)
oc1 = np.zeros(shape = [3])
for i in xrange(len(sol1)):
    oc1[i] = sol1[i]
    
sol2  =  Simult3(ob_axis[:,0],op_new[1,:],ob_fixed[1,:],V_unit(V1_norm),l_cr,l_rod)
oc2 = np.zeros(shape = [3])
for i in xrange(len(sol2)):
    oc2[i] = sol2[i]

sol3 =  Simult3(ob_axis[:,1],op_new[2,:],ob_fixed[2,:],V_unit(V2_norm),l_cr,l_rod)
oc3 = np.zeros(shape = [3])
for i in xrange(len(sol3)):
    oc3[i] = sol3[i]

sol4 =  Simult3(ob_axis[:,1],op_new[3,:],ob_fixed[3,:],V_unit(V2_norm),l_cr,l_rod)
oc4 = np.zeros(shape = [3])
for i in xrange(len(sol4)):
    oc4[i] = sol4[i]

sol5 =  Simult3(ob_axis[:,2],op_new[4,:],ob_fixed[4,:],V_unit(V3_norm),l_cr,l_rod)
oc5 = np.zeros(shape = [3])
for i in xrange(len(sol5)):
    oc5[i] = sol5[i]

sol6 =  Simult3(ob_axis[:,2],op_new[5,:],ob_fixed[5,:],V_unit(V3_norm),l_cr,l_rod)
oc6 = np.zeros(shape = [3])
for i in xrange(len(sol6)):
    oc6[i] = sol6[i]

def plotarr(ocix,ociy,ociz,pivot_arr):
    x1 = [ocix,pivot_arr[0]]
    y1 = [ociy,pivot_arr[1]]
    z1 = [ociz,pivot_arr[2]]
...
plotarr(oc1[0],oc1[1],oc1[2],ob_fixed[0,:])
plotarr(oc1[0],oc1[1],oc1[2],op_new[0,:])
plotarr(oc2[0],oc2[1],oc2[2],ob_fixed[1,:])
plotarr(oc2[0],oc2[1],oc2[2],op_new[1,:])

plotarr(oc3[0],oc3[1],oc3[2],ob_fixed[2,:])
plotarr(oc3[0],oc3[1],oc3[2],op_new[2,:])
plotarr(oc4[0],oc4[1],oc4[2],ob_fixed[3,:])
plotarr(oc4[0],oc4[1],oc4[2],op_new[3,:])

plotarr(oc5[0],oc5[1],oc5[2],ob_fixed[4,:])
plotarr(oc5[0],oc5[1],oc5[2],op_new[4,:])
plotarr(oc6[0],oc6[1],oc6[2],ob_fixed[5,:])
plotarr(oc6[0],oc6[1],oc6[2],op_new[5,:])
