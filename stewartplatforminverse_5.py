...
# For all odd cranks
oc_first_odd = np.zeros(shape = [1,3])
for i in xrange(3):
    v_per = V_cross2(V_unit(-ob_odd[:,i]),(V_unit(ob_axis[:,i] - ob_odd[:,i])))
    v_final = V_cross2(V_unit(-ob_odd[:,i]),V_unit(v_per))
    v_final_unit = V_unit(v_final)
    oc_first_odd = np.vstack((oc_first_odd,l_cr*v_final_unit))
    
oc_first_odd = oc_first_odd[1:,:]
    
# For all even cranks
oc_first_even = np.zeros(shape = [1,3])
for i in xrange(3):
    v_per = V_cross2(V_unit(-ob_even[:,i]),V_unit((ob_axis[:,i] - ob_even[:,i])))
    v_final = V_cross2(V_unit(-ob_even[:,i]),V_unit(v_per))
    v_final_unit = V_unit(v_final)
    oc_first_even = np.vstack((oc_first_even,l_cr*v_final_unit))
...
oc_first = uniarray(oc_first_even,oc_first_odd)
...
class Solver(object):
    ...
    def Simult3(self,op_new,ob_fixed,V2_norm,l_cr,l_rod,alfa):
        ...
        # Try two initial starting guesses
            ...
            f1 = (op_new[0] - oc1x)*(op_new[0] - oc1x) + (op_new[1] - oc1y)*(op_new[1] - oc1y) + (op_new[2] - oc1z)*(op_new[2] - oc1z) - 190*190
            f2 = (oc1x - ob_fixed[0])*(oc1x - ob_fixed[0]) + (oc1y - ob_fixed[1])*(oc1y - ob_fixed[1]) + (oc1z - ob_fixed[2])*(oc1z - ob_fixed[2]) - 450*450
    ...
    def plotlines(self,ob_fixed,op_new,oc):
        ...
        for j in xrange(6):
            ...
            plt.show()
    def checkside(self,oc,oc_first,ob_fixed): # Method used to check if the plotted point is on the correct side of the crank
        oc_new = oc
        for j in xrange(6):
            if (-1.5707 < V_ang(oc_new[j,:],oc_first[j,:])) and (V_ang(oc_new[j,:],oc_first[j,:])< 1.5707): # ANgle in radians
                oc_new[j,:] = oc_new[j,:]
            else:
                # Reflection of the point along the plane/line http://mathworld.wolfram.com/Reflection.html
                # x0 = [0,0,0]
                n_unit = V_unit(ob_fixed[j,:])
                n_unit = np.asmatrix(n_unit)
                #oc_new_r = n_unit*(np.dot(oc[j,:],n_unit))
                oc_new[j,:] = -oc_new[j,:] + 2*(n_unit*(np.dot(np.transpose(np.asmatrix(oc_new[j,:])),n_unit)))
        return oc_new
...
for i in  xrange(2):
    ...
    invkinsol = invkin.checkside(invkinsol_1,oc_first,ob_fixed)
    # Plotting the legs
    if invkinsol[0,0] != -1:
