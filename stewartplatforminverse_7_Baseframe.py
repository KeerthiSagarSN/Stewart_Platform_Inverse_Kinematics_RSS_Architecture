...
angle_alfa = np.math.degrees(np.math.atan(60.0/573.85))
...
for i in xrange(len(point_gen)):
    ...
    # Roll angle
    op_new = New_position(point_gen[i],op_home)    
...
class Solver(object):
    ...
    def Simult3(self,op_new,ob_fixed,ob_fixed_axis,op_c,l_cr,l_rod,alfa):
        try:
        # Try two initial starting guesses
            ...
            f3 = (oc1x - ob_fixed[0])*(ob_fixed_axis[0]) + (oc1y - ob_fixed[1])*(ob_fixed_axis[1]) + (oc1z - ob_fixed[2])*(ob_fixed_axis[2]) - 0
    ...
    def mainsolver(self,ob_axis,ob_fixed,ob_fixed_axis,op_c,op_new,l_cr,l_rod,alfa):
        ...
        for i in xrange(3):
            ...
            sol1 = []
            sol1 =  self.Simult3(op_new[counter,:],ob_fixed[counter,:],ob_fixed_axis[counter,:],op_c,l_cr,l_rod,alfa)
            ...
            sol2 = []    
            sol2  =  self.Simult3(op_new[counter,:],ob_fixed[counter,:],ob_fixed_axis[counter,:],op_c,l_cr,l_rod,alfa)
    ...
    def checkside(self,oc,oc_first,oc_first_axis,ob_fixed): # Method used to check if the plotted point is on the correct side of the crank
        ...
        for j in xrange(6):
            ...
            if (j%2.0 == 0):
                ...
                if (-(Pi/2.0) > angle_vec) and (angle_vec > -Pi): # ANgle in radians
                    teta1 = Pi - (-1*angle_vec)
...
for i in  xrange(len(point_gen)):
    ...
    invkinsol_1 = invkin.mainsolver(ob_axis,ob_fixed,ob_fixed_axis,op_c,op_new,l_cr,l_rod,alfa)
    ...
    if invkinsol[0,0] != -1:
        ...
        for r in xrange(6):
            dum1[r] = (V_ang_sign((invkinsol[r,:] - ob_fixed[r,:]),(oc_first[r,:] - ob_fixed[r,:]),oc_first_axis[r,:]))*1.0
