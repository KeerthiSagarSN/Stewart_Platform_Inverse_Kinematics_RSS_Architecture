...
class Solver(object):
    ...
    def Simult3(self,op_new,ob_fixed,ob_fixed_axis,ob_fixed_axis_1,op_c,oc_first,oc_first_axis,l_cr,l_rod,alfa):
        ...
        # Try two initial starting guesses
            ...
            '''
            def solvefunc(M,*data):
                x = M[0]
                y = M[1]
                z = M[2]
                op_new,ob_fixed,ob_fixed_axis,oc_first = data
                F = empty((3))
                F[0] = pow((op_new[0]-x),2) + pow((op_new[1]-y),2) + pow((op_new[2]-z),2) - 450*450
                F[1] = pow((ob_fixed[0]-x),2) + pow((ob_fixed[1]-y),2) + pow((ob_fixed[2]-z),2) - 190*190.0
                F[2] = (ob_fixed[0]-x)*(ob_fixed_axis[0]) + (ob_fixed[1]-y)*(ob_fixed_axis[1]) + (ob_fixed[2]-z)*(ob_fixed_axis[2]) - 0
                
                return F
            
            oc_guess = np.array([oc_first[0],oc_first[1],oc_first[2]])
            data = [op_new,ob_fixed,ob_fixed_axis,oc_first]
            M = fsolve(solvefunc,oc_guess,args = data)
            ...
            f1 = pow((op_new[0] - oc1x),2) + pow((op_new[1] - oc1y),2) + pow((op_new[2] - oc1z),2) - pow(450,2)
            
            f2 = pow((oc1x - ob_fixed[0]),2) + pow((oc1y - ob_fixed[1]),2) + pow((oc1z - ob_fixed[2]),2) - pow(190,2)
            ...
            if (-(Pi/2.0) < angle_vec) and (angle_vec < (Pi/2.0)): # ANgle in radians
                ...
                solution1 = solution[0]
            '''
            if np.linalg.norm((solution[0]-ob_fixed)) == 190:
                solution1 = solution[0]
            else:
                solution1 = solution[1]
            '''
